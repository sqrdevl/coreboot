...
int google_chromeec_swsync(void)
{
	...
	if (google_chromeec_read_hash(&resp, 0)) {
	...
	if (need_update) {
		...
		if (google_chromeec_read_hash(&resp, 1)) {
...
int google_chromeec_read_hash(struct ec_response_vboot_hash *hash, int force_recalc)
{
	struct chromeec_command cec_cmd;
	...
	do {
		...
		if (force_recalc && !recalc_requested) {
			hash->status = EC_VBOOT_HASH_STATUS_NONE;
		}
...
static int flash_write_cmd_version = -1;
...
int google_chromeec_flash_write_burst_size(void)
{
	...
	if (!google_chromeec_cmd_version_supported(EC_CMD_FLASH_WRITE, EC_VER_FLASH_WRITE)) {
		flash_write_cmd_version = 0;
		return EC_FLASH_WRITE_VER0_SIZE;
	} else {
		flash_write_cmd_version = EC_VER_FLASH_WRITE;
	}
	/*
			...
			uint32_t offset, uint32_t size)
{
	...
	cec_cmd.cmd_version = flash_write_cmd_version;
